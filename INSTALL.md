...
These instructions explain how to set up the tools required to build **rhythmtengoku**, which assembles the source files into a ROM.
...
>
>   note: this doesn't really apply to rhythmtengoku, delete?
</details>
...
## Windows 10/11 (WSL1)
WSL1 is the preferred terminal to build **rhythmtengoku**. The following instructions will explain how to install WSL1 (referred to interchangeably as WSL).
...
- Otherwise, **open WSL** and go to [Choosing where to store rhythmtengoku (WSL1)](#Choosing-where-to-store-rhythmtengoku-WSL1).
...
4. Certain packages are required to build rhythmtengoku. Install these packages by running the following command:

    ```bash
    sudo apt install build-essential binutils-arm-none-eabi git ffmpeg
    ```
    <details>
        <summary><i>Note...</i></summary>
...
### Choosing where to store rhythmtengoku (WSL1)
WSL has its own file system that's not natively accessible from Windows, but Windows files *are* accessible from WSL. So you're going to want to store rhythmtengoku within Windows.

For example, say you want to store rhythmtengoku (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps**. First, ensure that the folder already exists. Then, enter this command to **change directory** to said folder, where *\<user>* is your **Windows** username:
...
Otherwise, ask for help on the RHModding Discord (see [README.md](README.md)), or continue reading below for [Windows instructions using msys2](#windows-msys2).
...
- Otherwise, **open msys2** and go to [Choosing where to store rhythmtengoku (msys2)](#choosing-where-to-store-rhythmtengoku-msys2).
...
2. Certain packages are required to build rhythmtengoku. Install these by running the following two commands:

    ```bash
    pacman -Sy msys2-keyring
    pacman -S make gcc zlib-devel git ffmpeg
    ```
    <details>
        <summary><i>Note...</i></summary>
...
### Choosing where to store rhythmtengoku (msys2)
At this point, you can choose a folder to store rhythmtengoku into. If you're okay with storing rhythmtengoku in the user profile folder, then proceed to [Installation](#installation). Otherwise, you'll need to account for where rhythmtengoku is stored when changing directory to the rhythmtengoku folder.

For example, if you want to store rhythmtengoku (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps** (where *\<user>* is your **Windows** username), enter this command:
...
2.
    ...
    - Otherwise, **open Cygwin** and go to [Choosing where to store rhythmtengoku (Cygwin)](#choosing-where-to-store-rhythmtengoku-cygwin)
...
### Choosing where to store rhythmtengoku (Cygwin)

Cygwin has its own file system that's within Windows, at **C:\cygwin64\home\\_\<user>_**. If you don't want to store rhythmtengoku there, you'll need to account for where rhythmtengoku is stored when **changing directory** to the rhythmtengoku folder.

For example, if you want to store rhythmtengoku (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps**, enter this command, where *\<user>* is your **Windows** username:
```bash
cd c:/Users/<user>/Desktop/decomps
```
Note that the directory **must exist** in Windows. If you want to store rhythmtengoku in a dedicated folder that doesn't exist (e.g. the example provided above), then create the folder (e.g. using Windows Explorer) before executing the `cd` command.
...
2.  - If devkitARM is **not installed**, then go to [Installing devkitARM (macOS)](#installing-devkitarm-macos).
    - Otherwise, **open the Terminal** and go to [Choosing where to store rhythmtengoku (macOS)](#choosing-where-to-store-rhythmtengoku-macos)
...
### Choosing where to store rhythmtengoku (macOS)
At this point, you can choose a folder to store rhythmtengoku into. If you're okay with storing rhythmtengoku in the user folder, then proceed to [Installation](#installation). Otherwise, you'll need to account for where rhythmtengoku is stored when changing directory to the rhythmtengoku folder.

For example, if you want to store rhythmtengoku (and agbcc) in **~/Desktop/decomps**, enter this command to **change directory** to the desired folder:
```bash
cd Desktop/decomps
```
Note that the directory **must exist** in the folder system. If you want to store rhythmtengoku in a dedicated folder that doesn't exist (e.g. the example provided above), then create the folder (e.g. using Finder) before executing the `cd` command.
...
sudo apt install build-essential binutils-arm-none-eabi git ffmpeg
```
Then proceed to [Choosing where to store rhythmtengoku (Linux)](#choosing-where-to-store-rhythmtengoku-linux).

### Arch Linux
Run this command as root to install the necessary packages:
```bash
pacman -S base-devel arm-none-eabi-binutils git ffmpeg
```
Then proceed to [Choosing where to store rhythmtengoku (Linux)](#choosing-where-to-store-rhythmtengoku-linux).
...
>   Consider adding an exception for the `rhythmtengoku` and/or `decomps` folder in Windows Security using
...
1. If rhythmtengoku is not already downloaded (some users may prefer to download rhythmtengoku via a git client like GitHub Desktop), run:

    ```bash
    git clone https://github.com/arthurtilly/rhythmtengoku
    ```
    ...
    >   cd <folder where rhythmtengoku is to be stored>
    >   ```
    >   Where *\<folder where rhythmtengoku is to be stored>* is the path of the folder [where you chose to store rhythmtengoku](#Choosing-where-to-store-rhythmtengoku-WSL1). Then run the `git clone` command again.
    </details>

2. Install agbcc into rhythmtengoku. The commands to run depend on certain conditions. **You should only follow one of the listed instructions**:
- If agbcc has **not been built before** in the folder where you chose to store rhythmtengoku, run the following commands to build and install it into rhythmtengoku:
...
- **Otherwise**, if agbcc has been built before (e.g. if the git clone above fails), but was **last built on a different terminal** than the one currently used (only relevant to Windows, e.g. switching from msys2 to WSL1), then run the following commands to build and install it into rhythmtengoku:
...
- **Otherwise**, if agbcc has been built before on the same terminal, run the following commands to install agbcc into rhythmtengoku:
    ...
    <details>
        ...
        > If building agbcc or rhythmtengoku results in an error, try deleting the agbcc folder and re-installing agbcc as if it has not been built before.
    </details>

3. Once agbcc is installed, change directory back to the base directory where rhythmtengoku and agbcc are stored:

    ```bash
    cd ..
    ```

Now you're ready to [build **rhythmtengoku**](#build-rhythmtengoku)
## Build rhythmtengoku
If you aren't in the rhythmtengoku directory already, then **change directory** to the rhythmtengoku folder:
```bash
cd rhythmtengoku
```
To build **rhythmtengoku.gba** (Note: to speed up builds, see [Parallel builds](#parallel-builds)):
```bash
make
```
If it has built successfully you will have the output file **rhythmtengoku.gba** in your project folder.
<details>
